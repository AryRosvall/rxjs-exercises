...
import { StarwarsCharacter } from "./interfaces/starwarsCharacter.interface";
...
(() => {
  ...
  const character$: Observable<StarwarsCharacter> = getRequest(
    `${SW_API}/people/1/`
  );

  const getSpecie$ = (url) => {
    return url ? getRequest(url) : getRequest(`${SW_API}/species/1/`);
  };
  ...
  character$
    .pipe(
      /* Primera respuesta
      switchMap((character) => getSpecie$(character.species[0])) */
      switchMap((character) =>
        zip(of(character), getSpecie$(character.species[0])).pipe(
          map(([character, specie]) => ({
            character,
            specie,
          }))
        )
